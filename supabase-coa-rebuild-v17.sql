...
drop policy if exists "Public can view public COAs" on public.product_coas;
create policy "Public can view public COAs"
on public.product_coas
for select
using (is_public = true);

drop policy if exists "Authenticated admins manage COAs" on public.product_coas;
...
drop policy if exists "Authenticated admins upload product files" on storage.objects;
...
drop policy if exists "Authenticated admins update product files" on storage.objects;
...
drop policy if exists "Authenticated admins delete product files" on storage.objects;
