...
insert into public.merch_items
  (name,category,description,base_price,visible,sort_order)
select
  seed.name,
  seed.category,
  seed.description,
...
from (
  values
    ('Neon Peppers Logo Tee','Apparel','Classic Neon Peppers logo shirt.',25.00,1),
    ('Neon Peppers Trucker Hat','Headwear','Adjustable trucker hat with Neon Peppers branding.',28.00,2),
    ('Neon Peppers Hoodie','Apparel','Midweight Neon Peppers hoodie.',45.00,3),
    ('Neon Peppers Sticker Pack','Accessories','Assorted Neon Peppers vinyl stickers.',8.00,4)
) as seed(name,category,description,base_price,sort_order)
where not exists (
  select 1
  from public.merch_items current
